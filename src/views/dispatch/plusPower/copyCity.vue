...
import { aflcMediumOrderWeightedAdd } from "@/api/dispatch/plusPower.js";
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          standForm.forEach(el => {
            el.belongCityCode = this.form.belongCity;
            el.belongCityName = this.form.city;
            el.province = this.form.province;
          });
          aflcMediumOrderWeightedAdd(standForm)
            .then(res => {
              this.$message({
                type: "success",
                message: "操作成功！"
            ...
            .catch(err => {
              this.$message({
                type: "info",
                message: "操作失败，原因：" + (err.text || err.errorInfo)
              });
            });
        } else {
          return this.$message({
