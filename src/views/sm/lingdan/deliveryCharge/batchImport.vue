          ...
          <el-input v-model="form.excelFile" clearable>
            ...
            <label class="file-box" slot="append" for="file">
              ...
              <input
                ...
                ref="inputer"
                type="file"
                class="file-input Js_file-input"
                @change="getfileName"
              />
            </label>
          </el-input>
...
import {
  importDeliveryPriceExcel,
  deliveryPriceOutAgencyExcel
  ...
  methods: {
    ...
    getAreaAgency() {
      ...
      if (this.form.agencyAreaCode == "") {
        ...
        // console.log(this.form.agencyAreaCode);
        getAreaAgent(this.form.agencyAreaCode)
          .then(res => {
            setTimeout(() => {
              this.optionseAreaAgency = res.data;
              if (this.optionseAreaAgency == []) {
                this.form.carriersId = "";
                this.form.carriersName = "";
              }
            }, 200);
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          formdata.append(
            "carriersName",
            this.optionseAreaAgency.find(
              item => item.id == this.form.carriersId
            )["agencyName"]
          );
          formdata.append("useType", this.form.useType);
          // console.log('formdata',formdata)
          importDeliveryPriceExcel(formdata)
            .then(res => {
              this.$message({
                type: "success",
                message: "操作成功！"
    ...
    submitForm2(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.btnStatus = true;
          this.form.carriersName = this.optionseAreaAgency.find(
            item => item.id == this.form.carriersId
          )["agencyName"];
          let standFrom = objectMerge2(this.form);
          // console.log("standFromstandFrom", standFrom);
          this.$message.info("正在导出中...");
          deliveryPriceOutAgencyExcel(standFrom)
            .then(res => {
              ...
              NewDom.setAttribute(
                "download",
                standFrom.carriersName + "送货费定价.xlsx"
              );
