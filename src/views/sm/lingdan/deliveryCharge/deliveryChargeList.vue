<template>
  <div class="identicalStyle deliveryPriceList" v-loading="loading">
    ...
    >
      ...
      <el-form-item label="到达地" prop="commonArea">
    ...
    <div class="classify_info">
      <div class="btns_box">
        <el-button
          ...
          plain
          @click="handleClick('batchImport')"
          v-has:SERVICE_DISTRICTSALESPRICING_DISTRICTDELIVERYFEEANDPRICING_DISTRICTDELIVERYFEEANDPRICING_BATCHIMPORT
          :size="btnsize"
        >批量导入</el-button>
        <el-button
          type="primary"
          ...
          v-has:SERVICE_DISTRICTSALESPRICING_DISTRICTDELIVERYFEEANDPRICING_DISTRICTDELIVERYFEEANDPRICING_ADD
          :size="btnsize"
        >新增</el-button>
        <el-button
          type="primary"
          ...
          v-has:SERVICE_DISTRICTSALESPRICING_DISTRICTDELIVERYFEEANDPRICING_DISTRICTDELIVERYFEEANDPRICING_MODIFY
        >修改</el-button>
        <el-button
          type="primary"
          :size="btnsize"
          ...
          v-has:SERVICE_DISTRICTSALESPRICING_DISTRICTDELIVERYFEEANDPRICING_DISTRICTDELIVERYFEEANDPRICING_ENABLEDISABLE
        >启用/禁用</el-button>
        <el-button
          type="primary"
          icon="el-icon-upload"
          ...
          v-has:SERVICE_DISTRICTSALESPRICING_DISTRICTDELIVERYFEEANDPRICING_DISTRICTDELIVERYFEEANDPRICING_EXPORT
          :size="btnsize"
        >导出</el-button>
        <el-button
          type="primary"
          ...
          v-has:SERVICE_DISTRICTSALESPRICING_DISTRICTDELIVERYFEEANDPRICING_DISTRICTDELIVERYFEEANDPRICING_DOWNLOADIMPORTTEMPLATE
          :size="btnsize"
        >下载导入摸板</el-button>
      </div>
      <div class="info_news">
        ...
        >
          ...
          <el-table-column
            ...
            min-width="150"
          ></el-table-column>
          <el-table-column :show-overflow-tooltip="true" sortable label="到达地" min-width="200">
            <template slot-scope="scope">{{scope.row.province+scope.row.city+scope.row.area}}</template>
          </el-table-column>
          <el-table-column :show-overflow-tooltip="true" label="轻货(元/票)" min-width="250">
            <template slot-scope="scope">
              <div v-html="scope.row.smallGoods"></div>
            </template>
          </el-table-column>
          <el-table-column :show-overflow-tooltip="true" min-width="250" label="重货(元/票)">
            <template slot-scope="scope">
              <div v-html="scope.row.biggerGoods"></div>
            </template>
          </el-table-column>
          <el-table-column sortable prop="flag" label="状态" min-width="150">
            <template slot-scope="scope">
              <span
                :class="{blackName: scope.row.flag == '0',normalName :scope.row.flag == '1'}"
              >{{scope.row.flag=='0'?'禁用':'启用'}}</span>
            </template>
          </el-table-column>
          <el-table-column sortable prop="creater" label="最新操作人" min-width="150"></el-table-column>
          <el-table-column sortable prop="createTime" label="最新操作时间" min-width="160">
    ...
    <batchImport
      ...
      @downloading="handleClick('download')"
      :exportType="typeClass"
      :currentTitle="titleTxt"
      @close="firstblood"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    firstblood() {
      this.loading = true;
      collectFeesList2(this.listSystemObj)
        .then(res => {
      ...
      if (
        ...
        switch (type) {
          case "new":
            ...
            this.formtitle = "新增区代送货费定价";
            this.isModify = false;
            this.isBatch = false;
            break;
          case "revise":
            this.dialogFormVisible = true;
            this.formtitle = "修改区代送货费定价";
            this.isModify = true;
            this.isBatch = false;
          ...
          case "download":
            ...
            deliveryPriceTemplateExcel("0").then(res => {
              var blob = new Blob([res]);
              let href = window.URL.createObjectURL(blob);
              var NewDom = document.createElement("a");
              NewDom.setAttribute("href", href);
              NewDom.setAttribute("target", "_blank");
              NewDom.setAttribute("download", "区代送货费定价导入模板.xlsx");
                ...
                collectFeesStatus2(statusObj)
                  .then(res => {
                    ...
                    this.$message({
                      type: "success",
                      message:
                        this.selected.flag == "0" ? "启用成功!" : "禁用成功!"
                    });
                  })
                  .catch(err => {
                    this.$message({
