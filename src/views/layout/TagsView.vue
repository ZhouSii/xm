      ...
      >
        <div class="scroll-wrapper" ref="scrollWrapper" :style="{left: left + 'px'}">
          <router-link
            ...
            ref="tagIndex"
            v-if="showDash"
            :class="isActive(indexTag)?'active':''"
            to="/dashboard"
          >首页</router-link>
          ...
          <a
            ...
            @click="tagFull(tag.fullPath)"
            :key="tag.fullPath"
          >
            <span class="el-icon-refresh" title="刷新" @click.prevent.stop="refreshSelectedTag(tag)"></span>
    ...
    <div class="tags-view-more">
      ...
      <div class="contextmenu-box" v-show="visible">
        ...
        <ul class="contextmenu contextmenu-list">
          ...
          <router-link
            :class="isActive(tag, true)?'active-menu':''"
            tag="li"
            v-for="tag in Array.from(visitedViews)"
            :to="tag.fullPath"
            :key="tag.fullPath"
...
export default {
  ...
  methods: {
    tagFull(fullPath) {
      if (
        fullPath.indexOf("/users/carowner") >= 0 ||
        fullPath.indexOf("/users/shipper") >= 0
      ) {
    ...
    closeSelectedTag(view) {
      if (this.$route.meta.isClosed == false) {
        this.$route.meta.isClosed = true;
      }
