  ...
  methods: {
    firstblood() {
      ...
      orderStatistics(this.formAllData).then(res => {
        ...
        this.yMax = Math.max(...this.lineData);
        // console.log(this.yMax);
        this.naturalInflowData = res.data.naturalInflowFinishOrderNumList;
        this.barInit();
        this.loading = false;
      });
    },
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: function(params) {
            ...
            let arr1 = params.filter(
              (value, index) => index <= params.length / 2 - 1
            );
            arr1.map((item, i) => {
              relVal +=
            ...
            // let Markers2 =
            //   '<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:#43eec6;"></span>';
            // relVal +=
            //   params[0].name +
            //   "<br/>" +
            //   Markers1 +
