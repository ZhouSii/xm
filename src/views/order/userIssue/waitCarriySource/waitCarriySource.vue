  ...
  <div class="identicalStyle waitCarriySource" style="height:100%">
    <el-form :inline="true" class="demo-ruleForm classify_searchinfo">
      ...
      <el-form-item label="货主姓名">
        <el-input v-model="formAllData.shipperName"></el-input>
      </el-form-item>
      <el-form-item label="货主账号">
        <el-input v-model="formAllData.shipperMobile"></el-input>
      </el-form-item>
    ...
    <div class="classify_info">
      <div class="btns_box">
        <el-button
          ...
          icon="el-icon-remove"
          :size="btnsize"
          v-if="status.name=='shangjia'"
          @click="SoldOut"
          v-has:ORDER_ORDER_GOODS_MANAGE_PROCESS_IN_SHELF_THE_SHELVES
        >下架</el-button>
        <el-button
          type="primary"
          plain
          ...
          :size="btnsize"
          v-if="status.name=='xiajia'"
          @click="SoldIn"
          v-has:ORDER_ORDER_GOODS_MANAGE_PROCESS_OFF_SHELVES_THE_INSELF
        >上架</el-button>
        <el-button
          type="primary"
          plain
      ...
      <div class="info_news">
        ...
        >
          ...
          >
            <!-- <template
              slot-scope="scope"
            >{{(scope.row.shipperName==null?'':`${scope.row.shipperName}-`)+scope.row.shipperMobile}}</template>-->
          </el-table-column>
          <el-table-column label="出发地" prop sortable show-overflow-tooltip width="170">
            <template slot-scope="scope">
              ...
              >{{scope.row.viaAddressName1}}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="到达地"
          ...
          <el-table-column label="查看抢单人" sortable width="110">
            <template slot-scope="scope">
              <span class="BtnInfo" @click="pushOrderSerial3(scope.row)">
                查看抢单人
                <span class="RedCss">({{scope.row.interestedCount}})</span>
              </span>
            </template>
          </el-table-column>
...
import {
  ...
  OrderSoldOut,
  deletePointNetwork
} from "@/api/order/userIssue/userIssue.js";
  ...
  methods: {
    ...
    regionChange1(d) {
      ...
      if (d.area) {
        ...
        this.formAllData.startProvinceCityArea = d.province
          ? d.province.name
          : "";
      }
    },
    ...
    regionChange2(d) {
      ...
      if (d.area) {
        ...
        this.formAllData.endProvinceCityArea = d.province
          ? d.province.name
          : "";
      }
    },
    ...
    // 列表
    firstblood() {
      findFCLOrderInfoList(this.page, this.pagesize, this.formAllData).then(
        res => {
          this.dataTotal = res.data.totalCount;
          this.tableDataTree = res.data.list;
          this.tableDataTree.forEach(item => {
            item.useTime = parseTime(item.useTime, "{y}-{m}-{d} {h}:{i}:{s}");
            item.shipperInfo = item.shipperName
              ? item.shipperMobile
                ? item.shipperName + "-" + item.shipperMobile
                : item.shipperName
              : item.shipperMobile
              ? item.shipperMobile
              : "";
    ...
    delDataInformation() {
      this.$confirm("确定要删除吗？", "提示", {
        ...
        .then(() => {
          deletePointNetwork(this.ArrayData())
            .then(res => {
    ...
    pushOrderSerial(val) {
      this.$router.push({
        name: "货源详情",
        query: { orderSerial: val.orderSerial, type: "sourceDetail" }
      });
    },
    pushOrderSerial3(val) {
      this.$router.push({
        name: "货源详情",
        query: { orderSerial: val.orderSerial, type: "RobberyDetail" }
      });
    }
  },
  mounted() {
