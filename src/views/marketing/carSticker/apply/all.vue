<template>
    <div class="identicalStyle AllApplyCompnent" v-loading="loading">
            ...
            <div class="info_news">
                <el-table
                    ...
                    <el-table-column
                        ...
                        >
                        <template slot-scope="scope">
                            {{scope.row.isNeedPay === "1" ? "需要" : "不需要"}}
                        </template>
                    </el-table-column>
                    <el-table-column
                        prop="isSubmitFirstTrial"
                        sortable
                        min-width="150"
                        label="是否提交初审"
                        >
    ...
    data() {
        return {
            ...
            listSystemObj:{
                ...
                "pageSize": 20,
                "vo": {
                    applyStatus:'',//当前车贴状态(【待处理】显示：待处理（AF0880401）【已通过】显示：通过（AF0880402）【不通过】显示：车主取消（AF0880403）不符合条件（AF0880404） )
                ...
                switch (type) {
                    case 'export':
                        ...
                        carStickerApplyExcel(this.listSystemObj).then(res => {
                            ...
                            NewDom.setAttribute('download', '车贴申请上报全部数据'+parseTime(timestamp)+'.xlsx');
