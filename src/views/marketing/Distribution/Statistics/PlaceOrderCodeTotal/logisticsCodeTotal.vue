<template>
  <div class="identicalStyle PlaceOrderCodeTotal" v-loading="loading">
    ...
    >
      ...
      <el-form-item label="揽货时间" prop="chooseTime">
        <el-date-picker
          v-model="chooseTime"
          type="daterange"
          range-separator="-"
      ...
      <el-form-item label="下单码所属货主" prop="orderCodeUserNameOrPhoneOrPlateNumber">
        <el-input
          v-model="listSystemObj.vo.orderCodeUserNameOrPhoneOrPlateNumber"
          placeholder="手机号/姓名"
          clearable
        ></el-input>
      </el-form-item>
      <el-form-item label="运单号" prop="orderSerial">
    ...
    <div class="classify_info">
      <div class="btns_box">
        <el-button
          ...
          :size="btnsize"
          @click="importExcel"
          v-has:MARKETING_DISTRIBUTION_STATISTICAL_PLACE_ORDER_SUM_EXPORT_EXCEL
        >导出EXCEL</el-button>
      </div>
      <div class="info_news">
        <el-table
...
import {
  lclPlaceOrderCodeTotal,
  PlaceLclOrderCodeListExcel
...
export default {
  ...
  methods: {
    ...
    handleSearch(type) {
      this.listSystemObj.vo.useStartTime = this.chooseTime
        ? this.chooseTime[0]
        : "";
      this.listSystemObj.vo.useEndTime = this.chooseTime
        ? this.chooseTime[1]
        : "";
      if (type == "clear") {
    ...
    firstblood() {
      this.loading = true;
      lclPlaceOrderCodeTotal(this.listSystemObj)
        .then(res => {
    ...
    importExcel() {
      ...
      PlaceLclOrderCodeListExcel(0, 0, this.listSystemObj.vo).then(res => {
        var blob = new Blob([res]);
        this.href = window.URL.createObjectURL(blob);
        var NewDom = document.createElement("a");
        NewDom.setAttribute("href", this.href);
        NewDom.setAttribute("target", "_blank");
        NewDom.setAttribute("download", "发物流下单码下单汇总.xls");
