<template>
  <div class="identicalStyle" style="height:100%" v-loading="loading">
    ...
    <addOrModify
      ...
      :editType="type"
      :typetitle="typetitle"
      :shareActDialogVisible.sync="shareActDialogVisible"
      @getData="getDataList"
      :baseRewardCouponList="baseRewardCouponList"
    />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getMoreInformation() {
      getFoundationRewardList().then(res => {
        this.baseRewardCouponList = res.data;
      });
    },
    // getStrEnd(d) {
      ...
      if (!this.selectRowData && type !== "add") {
        ...
        switch (type) {
          case "add":
            this.type = "add";
            this.typetitle = "新增";
            this.shareActDialogVisible = true;
            break;
          case "edit":
            this.type = "edit";
            this.typetitle = "修改";
