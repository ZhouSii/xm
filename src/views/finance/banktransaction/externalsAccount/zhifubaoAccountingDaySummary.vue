...
import {
  aflcAliDownloadListFundBillSummary,
  aflcAliDownloadListFundBillDetailExcel
} from "@/api/finance/banktransaction";
export default {
  name: "zhifubaoAccountingDaySummary",
  components: {
    Pager
  },
  data() {
  ...
  methods: {
    firstblood() {
      this.loading = true;
      this.radio = "";
      aflcAliDownloadListFundBillSummary(this.page, this.pagesize, {
    ...
    importExcel() {
      ...
      if (!this.selectRowData) {
        ...
        aflcAliDownloadListFundBillDetailExcel(0, 0, paramsData).then(res => {
          ...
          NewDom.setAttribute(
            "download",
            "支付宝账务明细详情_" + this.selectRowData.createTimeDay + ".xls"
          );
